...
export default function Page() {
	...
	return (
		<div className="relative p-4  bg-gradient-primary-blue border-customblue border rounded-xl w-full mx-auto | md:w-[600px]">
			...
			<p className="mt-2">
				Heb je geen zin meer om elke minuut te kijken of je docent het cijfer er
				al op heeft gezet? Hier is de oplossing! Schrijf je in, en ontvang
			...
			<Columns margin={false}>
				<button
					...
					disabled={disabled}
					onClick={async (e) => {
						if (!username || !password) return setOutput("Ongeldige gegevens.");
						...
						fetch("https://laurensverhaar.nl/haganum-nuntia/api/account", {
							...
							.then(async (res) => {
								...
								if (res.status === 200) {
									...
									return setOutput(`Er is iets misgegaan: ${await res.text()}`);
							})
							.catch((error) => {
								setDisabled(false);
								setOutput(`Er is iets misgegaan: ${error}`);
